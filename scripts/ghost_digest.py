...
via the `ghst` CLI, compares against state/<site>/posts.json (yesterday's
snapshot), computes diffs for changed posts, asks DeepSeek V4-Flash for a
daily summary, and posts to the configured Telegram destination(s).
...
def fetch_posts(site: dict) -> list[dict]:
    """Use ghst CLI to fetch all published posts with plaintext."""
    alias = site["alias"]
    url = site["url"]
    token = site["token"]
    ...
    if auth_result.returncode != 0:
        print(f"[error] ghst auth failed for {alias}: {auth_result.stderr}", file=sys.stderr)
        return []

    # Fetch published posts with plaintext
    result = subprocess.run(
        ["ghst", "post", "list", "--limit", "all",
         "--filter", "status:published",
         "--formats", "plaintext",
         "--json",
         "--jq", ".posts[] | {slug, title, plaintext, updated_at, published_at, url, primary_author, feature_image}"],
        capture_output=True, text=True, timeout=120,
    )
    if result.returncode != 0:
        print(f"[error] ghst post list failed for {alias}: {result.stderr}", file=sys.stderr)
        return []
    ...
    for line in result.stdout.strip().split("\n"):
        ...
        try:
            ...
            # Normalize primary_author to string
            author = post.get("primary_author")
            if isinstance(author, dict):
                post["primary_author"] = author.get("name", "unknown")
            elif not author:
                post["primary_author"] = "unknown"
            posts.append(post)
...
def is_trivial_diff(diff_text: str) -> bool:
    ...
    if len(stripped) < MIN_DIFF_CHARS:
        return True
    # If removing all whitespace yields < 5 chars, it's trivial
...
def diff_posts(old_posts: dict[str, dict], new_posts: list[dict]) -> list[dict]:
    ...
    # Also detect deletions (post was removed/unpublished)
...
def process_site(site: dict, tokens: dict) -> bool:
    """Fetch, diff, summarize, notify for one site. Return True if changes were found."""
    alias = site["alias"]
    ...
    if not changes:
        ...
        # Still save the snapshot (timestamps may have drifted)
        save_state(alias, new_posts)
        return False
...
def main() -> int:
    ...
    # Sites can also come from GHST_SITES env (JSON array) — overrides or extends config
    env_sites = os.environ.get("GHST_SITES", "")
    if env_sites:
        try:
            env_sites_list = json.loads(env_sites)
            if isinstance(env_sites_list, list):
                # Merge: env sites override config sites by alias
